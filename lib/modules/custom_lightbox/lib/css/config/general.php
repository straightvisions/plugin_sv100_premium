<?php
	...
	if(is_array($lightboxes) && count($lightboxes) > 0) {
		foreach ($module->get_setting('lightbox')->get_data() as $lightbox) {
			...
			$properties						= array();
			$properties['display']			= $_s->prepare_css_property_responsive(array_map(function ($enable_background_layer) { return $enable_background_layer['enable_background_layer'] ? 'block' : 'none'; }, $lightbox['enable_background_layer']));
